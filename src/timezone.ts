...
import { defaultEmbed, TIMEZONES } from "./config";
...
export const timezone = async (message: Message) => {
  ...
  if (match) {
    ...
    const proposedZone = TIMEZONES.find(
      (item: { abbr: string }) => item.abbr === zone.toUpperCase()
    );
    ...
    if (proposedZone) {
      ...
      const availableZones = TIMEZONES.filter(
        (item: { abbr: string }) => item.abbr !== zone
      );

      embed.addField(mentioned, proposedZone!.zone);

      availableZones.forEach((item: { zone: any; abbr: any }) =>
        embed.addField(
          `${item.zone} (${item.abbr})`,
